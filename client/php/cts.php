 ...
 */
function replace_tree($file_path, $selector, $new_contents) {
...
function resolve_file($public_url) {
  echo "Resolve: " . $public_url . "\n";

  $me_path = $_SERVER["REQUEST_URI"];
  $public_path = parse_url($public_url, PHP_URL_PATH);

  $me_parts = explode(DIRECTORY_SEPARATOR, $me_path);
  $pub_parts = explode("/", $public_path);

//  while (
//    ((sizeof($me_parts) > 0) and (sizeof($pub_parts) > 0)) and
//    ($me_parts[0] == $pub_parts[0])) {
//    array_shift($me_parts);
//    array_shift($pub_parts);
//    }
//
//  // Verify that me_parts is just the filename now.
//  if (sizeof($me_parts) != 1) {
//    $me_str = implode(DIRECTORY_SEPARATOR, $me_parts);
//    throw new Exception("File resolution error:" . $me_str);
//  }
//
//  // Now pub_parts sould be relative to this file.
  //  //$other_file = implode(PATH_SEPARATOR, $pub_parts);
    $other_file = $pub_parts[sizeof($pub_parts) - 1];
    $scriptPath = realpath(dirname(__FILE__));
    $ret = implode(DIRECTORY_SEPARATOR, array($scriptPath, $other_file));
    print $ret . "\n";
    return $ret;
}

function incorporate_changes($request) {
  if (is_array($request['operations'])) {
    foreach ($request['operations'] as $operation) {
      $action = $operation['action'];
      $publicUrl = $operation['treeUrl'];
      $localFile = resolve_file($publicUrl);
